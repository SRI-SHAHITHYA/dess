...
import {
  EduModuleFormInput,
  eduModuleFormSchema,
} from '@/validators/create-edu-module.schema';
...
// main module form component for create and update module
export default function CreateEduModule({
  id,
  module,
  isModalView = true,
}: {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        // Filter only standalone categories (modules are for standalone categories)
        const standaloneCategories = categoriesData.filter(cat => cat.type === 'standalone');
        const categoryOptions = standaloneCategories.map((cat) => ({
          value: Number(cat.id),
          label: cat.name
        }));
  ...
  const onSubmit: SubmitHandler<EduModuleFormInput> = async (data) => {
    setLoading(true);
    try {
      ...
      router.push(routes.edu.modules);
    } catch (error: any) {
  ...
  return (
    <Form<EduModuleFormInput>
      validationSchema={eduModuleFormSchema}
      resetValues={reset}
      onSubmit={onSubmit}
      useFormProps={{
        mode: 'onChange',
          ...
          <div className="flex-grow pb-10">
            ...
            >
              <HorizontalFormBlockWrapper
                ...
                description={'Edit your module information from here'}
              ...
              <HorizontalFormBlockWrapper
                ...
                description="Upload your module image here"
                isModalView={isModalView}
              >
                <UploadZone
                  name="images"
